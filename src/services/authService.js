...
function getErrorMessage(error, fallback) {
  if (error.code === "ECONNABORTED") {
    return "Temps de réponse trop long, le serveur est peut-être en veille. Réessayez dans quelques secondes.";
  }
...
const AuthService = {
  register: async (username, email, password) => {
    try {
      ...
      return {
        success: false,
        message: getErrorMessage(error, "Erreur lors de l'inscription"),
        error,
      };
    }
  },
  ...
  login: async (email, password) => {
    try {
      ...
      return {
        success: false,
        message: getErrorMessage(error, "Identifiants incorrects"),
        error,
      };
    }
  },
...
// Export indépendant pour les tâches
export const updateTaskStatus = async (taskId, status) => {
  try {
    const response = await API.put(`/tasks/${taskId}/status`, {status});
    return {
    ...
    return {
      success: false,
      message: getErrorMessage(error, "Échec de la mise à jour du statut"),
      error,
    };
  }
};
